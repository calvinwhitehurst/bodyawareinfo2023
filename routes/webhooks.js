...
console.log("today: " + mysqlTimestamp)
console.log("time right now: " + moment().format('MMMM Do YYYY, h:mm:ss a'))
...
cron.schedule("0 57 22 * * *", function (){
  return new Promise(function (resolve, reject){
    ...
    req.end(function (res) {
      ...
      connection.query(
        "INSERT INTO `currency_rate` (`rate`, `date`) VALUES (" + res.body + ",'" + tomorrow +"');", 
        function(err){
          if (err) {
            //throw err;
            console.log(err);
    ...
    connection.query(
      ...
      function (err, rows) {
        for (var i =0; i < rows.length; i++) {
          connection.query(
            "INSERT IGNORE INTO `sales` (`store`, `sales_date`, `sales_total`) VALUES ('" + rows[i].id + "', '" + tomorrow + "', '0.00');");
        }
        if (err) {
          //throw err;
          console.log(err);
